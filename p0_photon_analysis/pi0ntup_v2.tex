...
\noindent  This writeup is a part of an Analysis Preservation and educational
 package with a focus  on the Electromagnetic Calorimeter (EMCal) of the PHENIX
detector. It aims to present a few basic techniques
...
all useable for deriving any real physics results.}
\end{abstract}
...
\item two ROOT macros described in Sec.\ref{code}
\end{itemize}
...
how to find an optimum between the two trends. 
...
The first Ntuple type is called {\it ggntuple} (gamma-gamma Ntuple).
...
\subsection{The Cluster Ntuples \it{(gnt)}}
The second Ntuple type is called {\it gnt} and contains information on
