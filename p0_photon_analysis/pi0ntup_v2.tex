...
\item Two ROOT macros designed to manipulate these data, as described in Sec.\ref{code}
\end{itemize}
...
\end{verbatim}
\noindent
\textbf{Caveat:} the ROOT macros were developed in the standard PHENIX
analysis environment which is based on ROOT version 5.34/36. You will need
to make adjustments to the code to make it run under a more modern version e.g.\,version 6.
Alternatively, you may obtain a Docker image of ROOT version 5.34 from
the PHENIX Collaboration Organization on Docker Hub:
\begin{verbatim}
